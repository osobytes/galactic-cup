...
| --- | --- | --- | --- | --- | --- |
| L-C | Zorin OS 18.1; Chrome 150.0.7871.124; NVIDIA RTX 2070 SUPER, driver 595.71.05 | Flow pass; performance needs clean rerun | Flow pass; performance fail | Flow pass; performance fail | Viewport/flow complete; physical gamepad and positive audio proof missing; letterboxing fails |
...
The corrected runner now emits both observations. Its Chrome probe found that
an 800×540 viewport stretches the canvas to 800×540 instead of centering an
800×450 canvas with 45 px bars.
...
| --- | ---: | ---: | --- | --- |
| Chrome 150 | 960×540 | 904.4 ms | Inconclusive | The t0 forced-GC probe overlapped the judged flow sample |
...
- Positive audible-playback telemetry requires a clean rerun with the reviewed
  collector; Firefox also needs the reviewed letterbox probe.
