...
t.describe("GOLISEO prototype content", function()
    ...
    t.it("pins the accepted initial family records", function()
        ...
        t.eq(unarmed.activation, "press")
        t.eq(unarmed.contact_kind, "melee")
        t.eq(unarmed.windup_ticks, 6)
        t.eq(unarmed.active_ticks, 4)
        t.is_true(not unarmed.held_active)
        t.eq(unarmed.recovery_ticks, 12)
        t.eq(unarmed.cooldown_ticks, 24)
        t.eq(unarmed.reach_px, 30)
        t.eq(unarmed.projectile_speed_px_per_second, nil)
        t.eq(unarmed.projectile_lifetime_ticks, nil)
        t.eq(unarmed.front_arc_degrees, 100)
        t.eq(unarmed.movement_multiplier, 0.8)
        t.eq(unarmed.unguarded_outcome.interruption_ticks, 10)
        t.eq(unarmed.unguarded_outcome.displacement_px, 8)
        t.is_true(unarmed.unguarded_outcome.ball_spill)
        t.eq(unarmed.guarded_recoil_px, 6)

        local guard = action_families.guard
        t.eq(guard.activation, "held")
        t.eq(guard.contact_kind, "guard")
        t.eq(guard.windup_ticks, 6)
        t.eq(guard.active_ticks, nil)
        t.is_true(guard.held_active)
        t.eq(guard.recovery_ticks, 9)
        t.eq(guard.cooldown_ticks, 0)
        t.eq(guard.reach_px, nil)
        t.eq(guard.projectile_speed_px_per_second, nil)
        t.eq(guard.projectile_lifetime_ticks, nil)
        t.eq(guard.front_arc_degrees, 120)
        t.eq(guard.movement_multiplier, 0.55)
        t.eq(guard.unguarded_outcome, nil)
        t.eq(guard.guarded_recoil_px, 0)

        local melee = action_families.light_melee
        t.eq(melee.activation, "press")
        t.eq(melee.contact_kind, "melee")
        t.eq(melee.windup_ticks, 12)
        t.eq(melee.active_ticks, 5)
        t.is_true(not melee.held_active)
        t.eq(melee.recovery_ticks, 21)
        t.eq(melee.cooldown_ticks, 42)
        t.eq(melee.reach_px, 42)
        t.eq(melee.projectile_speed_px_per_second, nil)
        t.eq(melee.projectile_lifetime_ticks, nil)
        t.eq(melee.front_arc_degrees, 75)
        t.eq(melee.movement_multiplier, 0.5)
        t.eq(melee.unguarded_outcome.interruption_ticks, 18)
        t.eq(melee.unguarded_outcome.displacement_px, 18)
        t.is_true(melee.unguarded_outcome.ball_spill)
        t.eq(melee.guarded_recoil_px, 6)

        local ranged = action_families.ranged
        t.eq(ranged.activation, "held_release")
        t.eq(ranged.contact_kind, "projectile")
        t.eq(ranged.windup_ticks, 18)
        t.eq(ranged.active_ticks, 1)
        t.is_true(not ranged.held_active)
        t.eq(ranged.recovery_ticks, 27)
        t.eq(ranged.cooldown_ticks, 60)
        t.eq(ranged.reach_px, nil)
        ...
        t.eq(ranged.unguarded_outcome.interruption_ticks, 12)
        t.eq(ranged.unguarded_outcome.displacement_px, 10)
        t.is_true(ranged.unguarded_outcome.ball_spill)
        t.eq(ranged.guarded_recoil_px, 6)
    end)
